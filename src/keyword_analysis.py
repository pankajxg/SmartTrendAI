...
def keyword_growth_score(df, time_column, keyword, text_column='processed'):
    ...
    df2['has_kw'] = df2[text_column].apply(lambda t: 1 if keyword in str(t).split() else 0)
    half = len(df2) // 2
    first = df2['has_kw'].iloc[:half].sum()
    second = df2['has_kw'].iloc[half:].sum()
...
def build_trend_table(df, text_column='processed', time_column=None, top_n=20):
    ...
    for term, count in top:
        growth = keyword_growth_score(df, time_column, term, text_column=text_column) if time_column else 0.0
