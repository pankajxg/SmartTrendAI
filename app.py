...
from src.keyword_analysis import build_trend_table
from src.utils import plot_bar
import matplotlib.pyplot as plt

st.set_page_config(page_title="SmartTrendAI", layout="wide", initial_sidebar_state='expanded')
...
if topic:
    ...
    st.write(filtered[['date', text_column, 'sentiment_label']].head(50))

st.header("Top Keyword Frequency Chart")
words_counts = [(r['keyword'], int(r['count'])) for _, r in trends.head(top_n).iterrows()]
if words_counts:
    fig = plot_bar(words_counts, title='Top keywords frequency')
    st.pyplot(fig)
